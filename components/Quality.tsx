...
export function Quality() {
  return (
    <section id="quality" className="bg-forest py-24 md:py-36">
      <div className="mx-auto max-w-[1440px] px-6 md:px-10">
        <Reveal className="max-w-2xl">
          <span className="label-caps text-[12px] text-yellow">Our Standard</span>
        ...
        <RevealGroup className="grid sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-14 mt-20">
          {qualityPillars.map((pillar, i) => {
            const Icon = icons[i];
            return (
              <motion.div key={pillar.title} variants={revealItem} className="border-t border-ivory/15 pt-7">
                <Icon size={28} strokeWidth={1.25} className={accentClasses[i]} />
                <h3 className="font-display text-xl md:text-2xl text-ivory mt-6">{pillar.title}</h3>
                <p className="text-ivory/60 font-light mt-3 text-sm leading-relaxed">
                  {pillar.description}
                </p>
              </motion.div>
            );
