        ...
        <div className="mx-auto max-w-[1440px] px-6 md:px-10">
          {product.category && (
            <Link
              ...
              className="label-caps text-[11px] text-forest/70 hover:text-forest"
            >
          ...
          <div className="grid md:grid-cols-12 gap-10 md:gap-14 mt-4">
            ...
            <div className="md:col-span-6">
              <h1 className="font-display text-3xl md:text-5xl leading-[1.05] text-near-black">
                {product.name}
              </h1>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-8">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 text-[13px] label-caps text-forest border border-forest/30 px-7 py-4 rounded-full hover:border-forest transition-colors duration-300"
                >
                ...
                <dl className="grid grid-cols-2 gap-x-6 gap-y-4 mt-10 pt-8 border-t border-charcoal/10">
                  {product.packSizes && (
                    <div>
                      ...
                      <dd className="text-sm text-near-black mt-1">{product.packSizes.join(", ")}</dd>
                    </div>
                  )}
                  {product.shelfLife && (
                    <div>
                      <dt className="label-caps text-[10px] text-charcoal/50">Shelf life</dt>
                      <dd className="text-sm text-near-black mt-1">{product.shelfLife}</dd>
                    </div>
                  )}
                  {product.storage && (
                    <div>
                      <dt className="label-caps text-[10px] text-charcoal/50">Storage</dt>
                      <dd className="text-sm text-near-black mt-1">{product.storage}</dd>
                    </div>
                  )}
                  {product.moq && (
                    <div>
                      <dt className="label-caps text-[10px] text-charcoal/50">MOQ</dt>
                      <dd className="text-sm text-near-black mt-1">{product.moq}</dd>
                    </div>
                  )}
                </dl>
              )}
                ...
                <div className="mt-8">
                  ...
                  <ul className="mt-3 space-y-1.5">
                    {product.benefits.map((benefit) => (
                      <li key={benefit} className="text-sm text-near-black/80">
                        • {benefit}
                      </li>
                    ))}
                  </ul>
              ...
              {product.applications && product.applications.length > 0 && (
                <div className="mt-6">
                  ...
                  <p className="text-sm text-near-black/80 mt-3">{product.applications.join(", ")}</p>
                </div>
              )}
              ...
              {product.ingredients && product.ingredients.length > 0 && (
                <div className="mt-6">
                  ...
                  <p className="text-sm text-near-black/80 mt-3">{product.ingredients.join(", ")}</p>
                </div>
              )}
                  ...
                  <dl className="mt-3 space-y-1.5">
                    {product.specifications.map((spec) => (
                      <div key={spec.label} className="flex gap-2 text-sm">
                        ...
                        <dd className="text-near-black/80">{spec.value}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
          ...
          {related.length > 0 && (
            <div className="mt-20 md:mt-28 pt-14 border-t border-charcoal/10">
              <h2 className="font-display text-2xl md:text-3xl text-near-black mb-8">
                Related products
              </h2>
              <ProductGrid
